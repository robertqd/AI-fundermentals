...
Agent Infra 是「让 Agent 能真正可靠运行、能操作真实世界、能被规模化托管」的那层底座——沙箱隔离、执行环境、编排引擎、运维接入，以及把这些能力焊到边缘现场的硬件网关，决定了系统能否从 demo 走向生产。本节盘点了 OpenHarness、Kagent、Agent Sandbox、Agentic Box 等主流方案的架构思路。
